...
This document provides a comprehensive reference table of [workspace](/langsmith/administration-overview#workspaces) and [organization](/langsmith/administration-overview#organizatios) operations and which roles can perform them. For an overview of LangSmith's RBAC system, role definitions, and permission concepts, refer to [Role-based access control](/langsmith/rbac).

This page lists every API operation in LangSmith along with:
...
|---------------------------|-------------------------------|
| **Core resources:**<br/>• [Projects](#projects-tracer-sessions): Organize traces and runs<br/>• [Runs](#runs-traces-and-spans): Individual execution traces<br/>• [Datasets](#datasets): Test datasets for evaluation<br/>• [Examples](#examples): Individual dataset examples<br/>• [Experiments](#experiments): Comparative experiments | **Core management:**<br/>• [Organization settings](#organization-settings): Org info and configuration<br/>• [Workspaces](#workspaces-organization-level): Workspace management<br/>• [Organization members](#organization-members): Member management<br/>• [Roles and permissions](#roles-and-permissions): Custom roles |
| **Monitoring and analysis:**<br/>• [Rules](#rules-run-rules--automations): Automated run rules<br/>• [Alerts](#alerts): Alert rules for monitoring<br/>• [Feedback](#feedback): Scores and labels on outputs<br/>• [Annotation Queues](#annotation-queues): Human review queues<br/>• [Charts](#charts-custom-dashboards): Custom visualizations | **Security and authentication:**<br/>• [SSO and authentication](#sso-and-authentication): Single sign-on setup<br/>• [SCIM](#scim-system-for-cross-domain-identity-management): Identity provisioning<br/>• [Access policies](#access-policies-abac---attribute-based-access-control): Attribute-based access control |
| **Development and configuration:**<br/>• [Prompts](#prompts-hub): Prompt templates (LangChain Hub)<br/>• [Deployments](#deployments): Deployment configurations<br/>• [MCP Servers](#mcp-model-context-protocol-servers): Model Context Protocol servers | **Billing and accounts:**<br/>• [Billing and payments](#billing-and-payments): Subscription management<br/>• [API keys and service accounts](#api-keys-and-service-accounts): Org-level keys |
...
- [User-level operations](#user-level-operations-no-workspaceorg-context): Operations for all authenticated users
- [Permission inheritance](#permission-inheritance): How roles inherit across org/workspaces
- [Special access flags](#special-access-flags): Public access, delegation, and service flags
